...
dataset ;imported household data
factor ;GIS units per NetLogo unit
n-turtles ;total number of turtles
pv-init ;transfer variable for setting initial PV share
roofsizes; transfer variable for setting the roof sizes from the csv
roof-sizes; transfer variable for setting the roof sizes without the header of the csv
...
to load_data
  clear-all
  ;; Load the shapefile
  set dataset gis:load-dataset "combined_household_coordinates.shp"
  ;scale the GIS geographic unit to netlogo unit for radius social network
  let world-envelope gis:envelope-of dataset
  ...
  set hh_fields gis:property-names dataset
  let feature-list (gis:feature-list-of dataset)


  foreach feature-list [ ?1 ->
    let hh gis:centroid-of ?1    ;location of the  households
    set roofsizes csv:from-file "combined_b3_opp_dak_schuin_values_vaals.csv"
    let location gis:location-of hh
    ifelse empty? location [
    ][
    create-households 1 [ ;; initialize households
      setxy item 0 location item 1 location
      set shape "house"
      set color blue
      set size 1.2
      let x (1 + (random(100 - 1 + 1)))
        ifelse x < 15 [set income (1 + random (10000 - 1 + 1)) ]
        [ifelse x > 14 and x < 30 [ set income (10000 + random (20000 - 10000 + 1)) ]
        [ifelse x > 29 and x < 44 [ set income (20000 + random (30000 - 20000 + 1)) ]
        [ifelse x > 43 and x < 56 [ set income (30000 + random (40000 - 30000 + 1)) ]
        [ifelse x > 55 and x < 66 [ set income (40000 + random (50000 - 40000 + 1)) ]
        [ifelse x > 65 and x < 74 [ set income (50000 + random (60000 - 50000 + 1)) ]
        [ifelse x > 73 and x < 82 [ set income (60000 + random (70000 - 60000 + 1)) ]
        [ifelse x > 81 and x < 82 [ set income (70000 + random (80000 - 70000 + 1)) ]
        [ifelse x > 88 and x < 95 [ set income (80000 + random (90000 - 80000 + 1)) ]
        [ set income (90000 + random (100000 - 90000 + 1)) ]
      ]
      ]]]]]]]]
    ]
  ]
    set use-hh 1  ;; hh are loaded s output can be saved
    set n-turtles count households
...
to setup
  ...
  ;csv:to-file csv_file_name (list "InputRandomSeed" "TPB_operationalization" "Financial_Information" "MF_Income" "Visibility" "Info_Costs" "Info_Costs_Revenue" "Info_Costs_Income" "Information_Threshold" "information_distribution" "information_threshold_value_value_value" "Income" "initial_pv_share" "probability_financial_information" "cost_time" "pv-yes?" "SE_importance_attitude" "SE_importance_control" "w_eco" "w_env" "w_cof" "w_soc" "pv-eco" "pv-env" "pv-cof" "pv-soc" "pv-inc")
...
;set pv-no and pv-yes for all utility functions
  ...
  ; initial PV share
    ...
    set esystem (list pv-init 0 0 0 0) ; esystems: [pv floor wall roof window] -- initially some of the hh have installed the technologies, here we deal only with pvoor wall roof window] -- initially some of the hh have installed the technologies, here we deal only with pv

; Assign income class based on income
...
ifelse income <= 40000 [
  ...
  ; Iterate over each household and update its roof size
  set roof-sizes but-first roofsizes
  set roof-size first item who roof-sizes
...
[
  ...
  set pv-cost-per-m2 1000 ; price in euro/m2 -- peak power = 1 kwp/m2 and PV price 2000 euro/kwp
  set pv-peak-power 1 ; this is max power in kw/m2
  set sunshine-hour 2209 ; the total sunshine hour in the netherlands
  set performance-ratio 0.6 ; this is the overall perfomance efficiency of the pv system
  set pv-lifetime 25 ; assumed life time of PV in years
  set grid-electricity-costs 0.26 ; this is the electricity price in euro/kwh in year 2012 based on CBS data
...
;;  random links
 ...
 ask turtles with [count link-neighbors = 0] [create-link-with one-of other turtles] ; lone turtles create a random link
  ask n-of (initial_pv_share * n-turtles) households [ set esystem replace-item 0 esystem 1]
...
set unc_level ((unc_level * pv-eco-util-prep * 2))] ;diff sizes here
 ...
 ;calculation of economic utility if Info_Costs == True
[  ifelse Info_Costs
  ...
  [ let random_hyp4 random-float 1
    ifelse random_hyp4 < probability_financial_information ; implementation probability barrier, full information for some agents
      ...
      ifelse Info_Costs_Income
      [set total-pv-cost-fin total-pv-cost + influence_cost_time * ((tot_rev / 21) + (income * 10000)) / 12 * cost_time];including hh's monthly income in payback calculation
...
to-report pv-util-inc [x]
  ...
  set pv-inc-util 1 * ((1 + exp((-1) *(normal * income) + 6 )))^ (-1) ; sigmoid function, input household income and "normal", which puts the household income in relation to the average income
  report pv-inc-util
end
...
to-report test-income-pv
  ifelse MF_Income[
    ...
    set probability_inc 1 * ((1 + exp((-1) *(normal * income) + 6 )))^ (-1) ; sigmoid function, input household income and "normal", which puts the household income in relation to the average income
      ifelse MF_Income_Barrier
    [set thres_prob MF_income_barrier_value]
...
; calculate multi-attribute utility function pro and conse PV installation
    ...
    ifelse Information_Threshold ; check if information cutoff is implemented
   [
     ifelse information_threshold_value > cost_time
...
182
csv_file_name
C:\\Users\\noaom\\PYTHON TRYOUT HEEDE\\file.csv
1
0
String
...
512
Nr of installations
count turtles with [pv-yes? = true]
17
1
11
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [pv-yes? = true]"
