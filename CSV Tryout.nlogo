...
to load-data
  ca
  read-csv-file "combined_household_coordinates.csv"
  set n-turtles count turtles
end

to read-csv-file [filename]
  file-open filename
  while [not file-at-end?] [
    let line file-read-line
    let values (csv:from-row line)  ; Split the line into a list of values using the csv extension

    ; Extract latitude and longitude
    let lat item 0 values
    let lon item 1 values

    ; Create turtles at the specified latitude and longitude
    create-turtles 1 [
      set shape "house"
      set color blue
      set size 1.5
      setxy precision lon 6 precision lat 6  ; Set the x and y coordinates with precision
    ]
  ]
  file-close
  set n-turtles count turtles
